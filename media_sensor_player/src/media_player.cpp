...
int cmedia_player::open_input_file(const char *filename)
{
    ...
    video_dst_file = fopen(filename, "wb+");
    if (!video_dst_file) {
    ...
    if ((ret = avformat_open_input(&ifmt_ctx, VIDEO_DEV, ifmt, NULL)) < 0) {
    ...
    av_dump_format(ifmt_ctx, 0, "video4linux2", 0);
    return ret;
...
void cmedia_player::video_display() {
    ...
    if (vp->frame) {
        ...
        av_image_copy(video_dst_data, video_dst_linesize,
                      (const uint8_t **)(vp->frame->data), vp->frame->linesize,
                      pix_fmt, width, height);
	printf(">>>>>=====11111 pix_fmt = %d  width = %d  height = %d\n",pix_fmt,width,height);
